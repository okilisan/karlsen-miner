...
__constant__ static const uint8_t heavyP[Plen] = {  };
 ...
 //__device__ hash512* const light_cache = reinterpret_cast<hash512*>(light_cache_num_items);
 //__device__ hash1024* full_dataset = reinterpret_cast<hash1024*>(full_dataset_num_items);
 
 //__shared__ hash512 light_cache[10];
 //__shared__ hash1024 full_dataset[10];
 
 __shared__ hash512* light_cache;
 __shared__ hash1024* full_dataset; 

 /*
 __device__ fishhash_context ctx = {
...
*/
/*
__shared__ fishhash_context ctx {
    light_cache_num_items,
    light_cache,
    full_dataset_num_items,
    full_dataset
...
__shared__ uint8_t cache_test[10];
...
/*
            if (sizeof(ctx.full_dataset) < 1) {
                printf("cuda malloc ========================================\n");
                cudaMalloc(&ctx.light_cache, 10*sizeof(hash512));
                cudaMalloc(&ctx.full_dataset, 10*sizeof(hash1024));
                //ctx.full_dataset = dataset;
                //ctx.light_cache = cache;
            }
            */

            //printf("heavy_hash debug 2 %lld >>>>>>>>>>>>>>>>>>> \n", sizeof(cache));
           ...
           fishhash_context ctx {
            ...
            //printf("heavy_hash debug 2.1 %llu >>>>>>>>>>>>>>>>>>> \n", sizeof(ctx.light_cache));

            memset(input, 0, 80);
            memcpy(input, hash_.hash, 32);
            hashFish(&ctx, hash_.hash, input);

            //printf("heavy_hash debug 3");
/*
